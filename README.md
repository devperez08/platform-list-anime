# EpiNeko

**EpiNeko** is a personal anime listing and progress tracking platform designed with a focus on visual discovery and fluid user experience.

Inspired by the dynamic layout of _JustWatch_, this project aims to provide a "vibe-centric" way to manage your watchlists without the clutter of traditional listing sites.

## 🛠️ Tech Stack

- **Framework:** Next.js 15 (App Router)
- **Styling:** Tailwind CSS + DaisyUI
- **Backend/Database:** Supabase (PostgreSQL)
- **Data Source:** Jikan API (v4)
- **State Management:** React Hooks

## ✨ Key Features

- **Visual Discovery:** Grid-based browsing with horizontal categories.
- **Real-time Tracking:** Update your progress (Watching, Completed, Dropped) instantly.
- **Secure Auth:** Social login powered by Supabase.
- **Responsive Design:** Optimized for both desktop and mobile "vibe coding" sessions.
